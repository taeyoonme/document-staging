Play 설정하기 {#customize-a-play}
====================

Play를 처음 생성한 뒤 좌측의 `General`의 하위 메뉴에서 Play와 관련된 설정을 할 수 있습니다. 이 설정은 Play를 만드는 데 있어 필수 사항은 아닙니다.

## 기본 정보(`General` > `기본 정보`) {#setting}

![](../images/create-plays-with-play-builder/customize-a-play1.jpg)
...
{% hint style='tip' %}
현재는 AudioPlayer Interface만 지원합니다. 
자세한 내용은 [AudioPlayer Interface 지원하는 Play 만들기](/create-plays-with-play-builder/create-a-play-with-audioplayer.md)를 참고하세요.
{% endhint %}


![](../images/create-plays-with-play-builder/customize-a-play2.jpg)
...
**⑤ Prompt 발화 옵션**   
음성 합성의 속성을 조절하는 옵션입니다. 발화 속도, 음의 높낮이, 문장 간 묵음의 길이, 끊어 읽을 때의 묵음 길이를 각각 설정할 수 있습니다. 
이 페이지에서의 설정은 이 Play 내에 모든 prompt 발화 옵션을 일괄 조정하는 옵션입니다. `발화 옵션 적용하기` 버튼을 클릭한 이후에 만들어진 prompt부터 옵션이 적용되며, 각 문장 우측의 <i class="fa fa-sliders"></i> 버튼을 통해 확인이 가능합니다.  

{% hint style='tip' %}
발화 옵션에 대한 자세한 내용은 [발화 옵션 사용하기](/create-plays-with-play-builder/use-prompts.md#use-utterance-options)를 참고하세요.
{% endhint %}

<br>
## 외부 연동 서버 설정 및 예외 상황 관리(`General` > `외부 서버 연결 정보`){#setting-backend-proxy}

![](../images/create-plays-with-play-builder/customize-a-play3.jpg)
...
![](../images/create-plays-with-play-builder/ch3_321_c04.png)
<br>
## 빌드/History(`General` > `빌드 / History`)

빌드 이력(History)을 확인할 수 있습니다. 또한, 이 페이지에서도 `Play 빌드` 버튼을 이용하여 Play를 빌드할 수 있습니다. 

![](../images/create-plays-with-play-builder/ch3_321_c05.png)
