...
| :--- | :--- | :--- | :--- | :--- | :--- |
| logo | ImageObject | Y | 투명 배경의 이미지로 제공해야 함\(PNG 타입\) | 50x50 가로 Size Flexible | STB2부터 적용 예정 사이즈 미정 |
...
| size | string | N | X\_SMALL, SMALL, MEDIUM, LARGE, X\_LARGE 화면 크기별 최적화를 위해 size 정보를 넣는 것을 권장합니다. Value별 권장 사이즈는 아래의 표를 참고하세요. |
...
| :--- | :--- | :--- | :--- |
| image | ImageObject | N |  |
| color | string | N | color 형식 \(RGB\) &gt;default - \#000000 |
| opacity | number | N | NU300에만 유효하며, default값은 0.7 |
...
| style | json | N | 기본적으로는 상위 스타일\(directive 등\)을 따르며, 현재 FullText1,2의 body text에만 적용 가능. CSS 속성은 모두 가능하며, 다음의 값들을 가질 수 있다. text-align : left, center, right opacity : 0 ~ 1 display : block, inline, none margin : 10px  사용예. { "text-align":"center", "display": "block"} |
| text | string | Y |  |
...
| listItems.header | TextObject | Y | 본문 제목 - 최대 1줄까지 표현 \(넘는 문자는 ... 처리\) |
| listItems.body | TextObjects \(list of TextObject\) | Y | 본문 내용 - 줄바꿈 허용 / 최대 4줄 - footer 정의 시 3줄 제한 |
| listItems.footer | TextObject | N | 보조설명 - 최대 1줄까지 표현 \(넘는 문자는 ... 처리\) |
